...
static int Global_DTClass_Used = false;
...
int pos;
	...
	if ( lvo == NULL )
	{
printf( "Unknown DTClass libcall %d at $%08x\n", val, ms->ms_MemoryAdr );
	}
...
int len;
	...
	if ( error )
	{
		printf( "%s:%04d: Error Writting DTClass lvo's\n", __FILE__, __LINE__ );
		goto bailout;
	}
	...
	while( LVOS[pos].Name )
	{
		if ( LVOS[pos].Used )
		{
			...
			if ( error )
			{
				printf( "%s:%04d: Error Writting DTClass Data\n", __FILE__, __LINE__ );
				goto bailout;
			}
		}
