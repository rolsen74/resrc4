...
static int Global_Ramdrive_Used = false;
...
{
	{ 0, -42, "_LVOKillRAD0", NULL },
	{ 0, -48, "_LVOKillRAD", NULL },
	{ 0, 0, NULL, NULL }
};
...
int pos;
	...
	if ( lvo == NULL )
	{
printf( "Unknown Ramdrive libcall %d at $%08x\n", val, ms->ms_MemoryAdr );
	}
...
int len;
	...
	if ( error )
	{
		printf( "%s:%04d: Error Writting Ramdrive lvo's\n", __FILE__, __LINE__ );
		goto bailout;
	}
	...
	while( LVOS[pos].Name )
	{
		if ( LVOS[pos].Used )
		{
			...
			if ( error )
			{
				printf( "%s:%04d: Error Writting Ramdrive Data\n", __FILE__, __LINE__ );
				goto bailout;
			}
		}
