...
static int Global_Keymap_Used = false;
...
{
	{ 0, -30, "_LVOSetKeyMapDefault", NULL },
	{ 0, -36, "_LVOAskKeyMapDefault", NULL },
	{ 0, -42, "_LVOMapRawKey", NULL },
	{ 0, -48, "_LVOMapANSI", NULL },
	{ 0, 0, NULL, NULL }
};
...
int pos;
	...
	if ( lvo == NULL )
	{
printf( "Unknown Keymap libcall %d at $%08x\n", val, ms->ms_MemoryAdr );
	}
...
int len;
	...
	if ( error )
	{
		printf( "%s:%04d: Error Writting Keymap lvo's\n", __FILE__, __LINE__ );
		goto bailout;
	}
	...
	while( LVOS[pos].Name )
	{
		if ( LVOS[pos].Used )
		{
			...
			if ( error )
			{
				printf( "%s:%04d: Error Writting Keymap Data\n", __FILE__, __LINE__ );
				goto bailout;
			}
		}
