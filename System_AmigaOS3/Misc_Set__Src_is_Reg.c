...
enum RS4ErrorCode
Misc_Set__Src_is_Reg ( struct AmigaOS3_Misc_Move_GetSetStruct * gss, struct M68kRegister * dst_reg, RS4Trace * rt, MEM mem )
{
	...
	if ( ( dst_rl ) && ( ! dst_rl->rl_UserLocked ) )
	{
		switch ( src_reg->mr_Type1 )
		{
			...
			case RRT_Library:
			{
				...
				if ( ( dst_rl->rl_Type1 == RS4LabelType_Pointer ) && ( dst_rl->rl_Type2 == RS4LabelPtrType_Library )
					 && ( dst_rl->rl_Type3 == src_reg->mr_Type2 ) )
				{
					// Same type, all okay
