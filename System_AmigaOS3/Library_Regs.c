...
enum RS4FuncStat
AmigaOS3_FindLibFunc_Regs ( enum RS4ErrorCode * errcode, RS4Trace * rt, struct _AOS3_RegStruct * rs )
{
		...
		switch ( qregs->Type3 )
		{
			case AOS3_RegType_Code:
			{
				...
				else
				{
					if ( DoVerbose > 2 )
					{
						printf ( "%s:%04d: hmm 1 : Type1 %d : Type2 %d : Type3 %d :\n", __FILE__, __LINE__, rl->rl_Type1,
								 rl->rl_Type2, rl->rl_Type3 );
					}
				}
				break;
			}
			...
			case AOS3_RegType_String:
			{
				...
				if ( ( rl->rl_Type1 == RS4LabelType_Unset ) || ( rl->rl_Type1 == RS4LabelType_String ) )
				{
			...
			case AOS3_RegType_Struct:
			{
				...
				if ( ( rl->rl_Type1 == RS4LabelType_Unset ) || ( rl->rl_Type1 == RS4LabelType_Struct ) )
				{
