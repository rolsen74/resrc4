...
static int Global_Colorwheel_Used = false;
...
{
	{ 0, -30, "_LVOConvertHSBToRGB", NULL },
	{ 0, -36, "_LVOConvertRGBToHSB", NULL },
	{ 0, 0, NULL, NULL }
};
...
int pos;
	...
	if ( lvo == NULL )
	{
printf( "Unknown Colorwheel libcall %d at $%08x\n", val, ms->ms_MemoryAdr );
	}
...
int len;
	...
	if ( Global_Colorwheel_Used == false )
	{
		error = false;
		goto bailout;
	}
	...
	if ( error )
	{
		printf( "%s:%04d: Error Writting Colorwheel lvo's\n", __FILE__, __LINE__ );
		goto bailout;
	}
	...
	while( LVOS[pos].Name )
	{
		if ( LVOS[pos].Used )
		{
			...
			if ( error )
			{
				printf( "%s:%04d: Error Writting Colorwheel Data\n", __FILE__, __LINE__ );
				goto bailout;
			}
		}
