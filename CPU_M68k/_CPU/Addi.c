...
enum RS4DecodeStat M68kCmd_ADDI( enum RS4ErrorCode *errcode, RS4Trace *rt )
{
...
S32 size;
	...
	switch( size )
	{
		case 0:
		{
			rt->rt_Container.Hunk.ms_Str_Opcode = ( ShortOpcodes ) ? "Add.b" : "Addi.b" ;
		...
		case 1:
		{
			rt->rt_Container.Hunk.ms_Str_Opcode = ( ShortOpcodes ) ? "Add.w" : "Addi.w" ;
		...
		case 2:
		{
			rt->rt_Container.Hunk.ms_Str_Opcode = ( ShortOpcodes ) ? "Add.l" : "Addi.l" ;
		...
		default:
		{
			printf( "Unsupported 'Addi' Opcode (Mode: %d)\n", size );
