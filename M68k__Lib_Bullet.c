...
static int Global_Bullet_Used = false;
...
{
	{ 0, -30, "_LVOOpenEngine", NULL },
	{ 0, -36, "_LVOCloseEngine", NULL },
	{ 0, -42, "_LVOSetInfoA", NULL },
	{ 0, -48, "_LVOObtainInfoA", NULL },
	{ 0, -54, "_LVOReleaseInfoA", NULL },
	{ 0, 0, NULL, NULL }
};
...
int pos;
	...
	if ( lvo == NULL )
	{
printf( "Unknown Bullet libcall %d at $%08x\n", val, ms->ms_MemoryAdr );
	}
...
int len;
	...
	if ( error )
	{
		printf( "%s:%04d: Error Writting Bullet lvo's\n", __FILE__, __LINE__ );
		goto bailout;
	}
	...
	while( LVOS[pos].Name )
	{
		if ( LVOS[pos].Used )
		{
			...
			if ( error )
			{
				printf( "%s:%04d: Error Writting Bullet Data\n", __FILE__, __LINE__ );
				goto bailout;
			}
		}
