...
int dst;
	...
	ms->ms_Str_Opcode = "FSAbs.x";
	ms->ms_OpcodeSize = 4;

//	printf( "Unsupported 'FSAbs1' Opcode at %08x\n", ms->ms_MemoryAdr );
...
int len;
	...
	switch( src )
	{
		...
		case 6:
		{
			...
			ms->ms_Str_Opcode = "FSAbs.b";
			break;
		}

		default:
		{
			printf( "Unsupported 'FSAbs2' Opcode (Size %d) at %08x\n", src, ms->ms_MemoryAdr );
