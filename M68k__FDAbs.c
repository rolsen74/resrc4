...
int dst;
	...
	ms->ms_Str_Opcode = "FDAbs.x";
	ms->ms_OpcodeSize = 4;

//	printf( "Unsupported 'FDAbs1' Opcode at %08x\n", ms->ms_MemoryAdr );
...
int len;
	...
	switch( src )
	{
		...
		case 6:
		{
			...
			ms->ms_Str_Opcode = "FDAbs.b";
			break;
		}

		default:
		{
			printf( "Unsupported 'FDAbs2' Opcode (Size %d) at %08x\n", src, ms->ms_MemoryAdr );
