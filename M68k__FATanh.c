...
int dst;
	...
	ms->ms_Str_Opcode = "FATanh.x";
	ms->ms_OpcodeSize = 4;

//	printf( "Unsupported 'FATanh1' Opcode at %08x\n", ms->ms_MemoryAdr );
...
int len;
	...
	switch( src )
	{
		...
		case 6:
		{
			...
			ms->ms_Str_Opcode = "FATanh.b";
			break;
		}

		default:
		{
			printf( "Unsupported 'FATanh2' Opcode (Size %d) at %08x\n", src, ms->ms_MemoryAdr );
