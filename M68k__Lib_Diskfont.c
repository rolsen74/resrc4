...
static int Global_Diskfont_Used = false;
...
{
	{ 0, -30, "_LVOOpenDiskFont", NULL },
	{ 0, -36, "_LVOAvailFonts", NULL },
	{ 0, -42, "_LVONewFontContents", NULL },
	{ 0, -48, "_LVODisposeFontContents", NULL },
	{ 0, -54, "_LVONewScaledDiskFont", NULL },
	{ 0, 0, NULL, NULL }
};
...
int pos;
	...
	if ( lvo == NULL )
	{
printf( "Unknown Diskfont libcall %d at $%08x\n", val, ms->ms_MemoryAdr );
	}
...
int len;
	...
	while( LVOS[pos].Name )
	{
		if ( LVOS[pos].Used )
		{
			...
			if ( error )
			{
				printf( "%s:%04d: Error Writting DiskFont Data\n", __FILE__, __LINE__ );
				goto bailout;
			}
		}
