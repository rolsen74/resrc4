...
RS4Text_ScanSection ( enum RS4ErrorCode * errcode, RS4FileSection * sec )
{
...
static enum RS4FuncStat
RS4Text_Scan ( enum RS4ErrorCode * errcode, RS4FileHeader * fh )
{
	enum RS4ErrorCode ec;
	enum RS4FuncStat  fs;
	RS4FileSection *  sec;
...
static enum RS4FuncStat
RS4ScanSecJumps ( enum RS4ErrorCode * errcode, RS4Trace * rt )
{
	enum RS4ErrorCode ec;
	enum RS4FuncStat  fs;
	RS4FileSection *  sec;
...
static enum RS4FuncStat
RS4ScanForJumps ( enum RS4ErrorCode * errcode, RS4Trace * rt, RS4FileHeader * fh )
{
	enum RS4ErrorCode ec;
	enum RS4FuncStat  fs;
	RS4FileSection *  sec;
...
static enum RS4FuncStat
RC4TraceBrance ( enum RS4ErrorCode * errcode, RS4Trace * rt, RS4Brance * rb )
{
	...
	if ( ( ! rt ) || ( ! rt->rt_File ) || ( ! rt->rt_Section ) || ( ! rb ) )
	{
	...
	{
		ERR_CHK ( RS4_Find_LabelAdr ( &ec, rt->rt_File, &rl, adr + pos, __FILE__ ) )
	...
	while ( TRUE )
	{
		...
		// printf( "---tbrance 5 : %lx %lx %p %p\n", pos, sec->rfs_MemoryAdr, sec->rfs_MemoryBuf, sec->rfs_MemoryType );
		// fflush(stdout);
		...
		if ( rt->rt_CPU.M68k.mt_JmpRegister.mr_Type1 != RRT_Unknown )
		{
			switch ( rt->rt_CPU.M68k.mt_JmpRegister.mr_Type1 )
			{
				case RRT_Label:
				{
					...
					ERR_CHK ( RS4_Find_LabelAdr ( &ec, rt->rt_File, &rl, jmpadr, __FILE__ ) )
...
enum RS4FuncStat
RS4Trace_File ( enum RS4ErrorCode * errcode, RS4FileHeader * fh )
{
	enum RS4ErrorCode ec;
	enum RS4FuncStat  fs;
	RS4Brance *		  rb;
