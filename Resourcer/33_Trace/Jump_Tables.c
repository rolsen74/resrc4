...
-- Table #26 - lha v2.15
	0x72,0,					// Moveq.l		#20,D1						; 7214
	0xb2,0x80,				// Cmp.l		D0,D1						; b280
	0x65,0,					// Bcs.b		Lab_05FC					; 654c
	0x30,0x3b,0x0a,0x06,	// Move.w		(i16_L05F9,PC,D0.l*2),D0	; 303b 0a06
	0x4e,0xfb,0x00,0x02		// Jmp			(i16_L05F9,PC,D0.w)			; 4efb 0002

-- Table #27 - lha v2.15
	0x72, 0,				// Moveq.l		#69,D1						; 7245
	0xb2, 0x80,				// Cmp.l		D0,D1						; b280
	0x65, 0x00, 0, 0,		// Bcs.w		Lab_05FC					; 6500 0120
	0x30, 0x3b, 0x0a, 0x06, // Move.w		(i16_L05F9,PC,D0.l*2),D0	; 303b 0a06
	0x4e, 0xfb, 0x00, 0x02	// Jmp			(i16_L05F9,PC,D0.w)			; 4efb 0002

-- Table #28 - lha v2.15
	0x72, 0,				// Moveq.l		#10,D1						; 720a
	0xb2, 0x88,				// Cmp.l		A0,D1						; b288
	0x65, 0x00, 0, 0,		// Bcs.w		Lab_0262					; 6500 0098
	0x30, 0x3b, 0x8a, 0x06, // Move.w		(i16_L0258,PC,A0.l*2),D0	; 303b 8a06
	0x4e, 0xfb, 0x00, 0x02	// Jmp			(i16_L0258,PC,D0.w)			; 4efb 0002
...
enum RS4JumpStat
RS4JumpTable_Rel16 ( enum RS4ErrorCode * errcode, RS4FileHeader * fh, S64 tabel_adr, S64 relative_adr, S32 entries )
{
...
static struct JTable Table1 = {
	...
	{ 0xff, 0xff, 0x00, 0x00, 0x00, 0x00, 0xff, 0xff, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff },
};
...
static struct JTable Table4 = {
	...
	{ 0xff, 0xff, 0x00, 0x00, 0xff, 0xff, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff },
};
...
static struct JTable Table6 = {
	...
	{ 0xff, 0xff, 0x00, 0x00, 0xff, 0, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff },
};
...
static struct JTable Table10 = {
	...
	{ 0xff, 0xff, 0x00, 0x00, 0xff, 0x00, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff },
};
...
static struct JTable Table12 = {
	...
	{ 0xff, 0xff, 0x00, 0x00, 0x00, 0x00, 0xff, 0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff },
};
...
static struct JTable Table13 = {
	...
	{ 0xff, 0xff, 0x00, 0x00, 0x00, 0x00, 0xff, 0x00, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff },
};
...
static struct JTable Table26 = {
	...
	{ 0xff, 0x00, 0xff, 0xff, 0xff, 0x00, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff },
};

// --

static struct JTable Table27 = {
	+4,		 // Tabel Pos
	+4,		 // Relative Pos
	-11,	 // EntryPos
	ET_Pos8, // EntryType
...
static struct JTable Table29 = {
	...
	14,		 // Data Size
	{
		0x72, 0,				// Moveq.l		#56,D1						; 7238
		0xb2, 0x80,				// Cmp.l		D0,D1						; b280
		0x65, 0,				// Bcs.b		Lab_0732					; 65ee
		0x30, 0x32, 0x0a, 0x00, // Move.w		(0,A2,D0.l*2),D0			; 3032 0a00
		0x4e, 0xfb, 0x00, 0x02	// Jmp			(i16_L05F9,PC,D0.w)			; 4efb 0002
	},
	{ 0xff, 0x00, 0xff, 0xff, 0xff, 0x00, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff },
};
...
static struct JTable * JumpTables[] = {
	...
	&Table21, &Table22, &Table23, &Table24, &Table25, &Table26, &Table27, &Table28, &Table29, NULL,
};

// --

static enum RS4JumpStat
check_JumpTable ( enum RS4ErrorCode * errcode, RS4Trace * rt, struct JTable * jt, S32 count )
{
	enum RS4ErrorCode ec;
	enum RS4JumpStat  js;
	RS4FileSection *  sec;
...
#if 0
{
	...
	switch ( jt->EntryType )
	{
		...
		{
			pos2	= off + jt->EntryPos;
			entries = ( ( mem[pos2 + 0] << 8 ) | ( mem[pos2 + 1] << 0 ) );
			break;
		}
		...
		{
			pos2	= off + jt->EntryPos;
			entries = ( ( mem[pos2 + 0] << 24 ) | ( mem[pos2 + 1] << 16 ) | ( mem[pos2 + 2] << 8 ) | ( mem[pos2 + 3] << 0 ) );
			break;
		}
	...
	entries += jt->EntryAdjust;

#warning todo check mem overflow
	// Simple check that we have a reasonal value
	if ( entries > 1000 )
	{
		ec = RS4ErrStat_Okay;
		js = RS4JumpStat_Skip;
	...
	if ( ( DoVerbose > 2 ) && ( pos1 & 1 ) )
	{
		printf ( "%s:%04d: Jump Table : Odd Address : $%" PRIx64 "\n", __FILE__, __LINE__, pos1 );
	}

	memset ( &type[pos1], RS4MT_Code, jt->DataSize );
...
enum RS4FuncStat
RS4Trace_JumpTable ( enum RS4ErrorCode * errcode, RS4Trace * rt )
{
	enum RS4ErrorCode ec;
	enum RS4FuncStat  fs;
	enum RS4JumpStat  js;
