...
static U32 LabelCount;

static enum RS4FuncStat
RS4AdjustLabels ( enum RS4ErrorCode * errcode, RS4FileSection * sec )
{
	...
	while ( ( CurLabel ) && ( CurLabel->rl_Offset <= size ) )
	{
		if ( CurLabel->rl_Name[0] == 0 )
		{
			...
			else
			{
				...
				{
					LabelCount++;
					snprintf ( labname, MAX_LabelName - 1, fmt, lab2, LabelCount );

					ERR_CHK ( RS4_Find_LabelName ( &ec, sec->rfs_File, &TmpLabel, labname ) )
				...
				ERR_CHK ( RS4_Add_LabelName ( &ec, sec->rfs_File, CurLabel, labname ) )
			}
		}
	...
	if ( ( CurLabel ) && ( CurLabel->rl_Offset > size ) )
	{
		...
		if ( EndLabel->rl_Name[0] == 0 )
		{
			...
			{
				LabelCount++;
				snprintf ( labname, MAX_LabelName - 1, "%s%04X", "Lab_", LabelCount );
			...
			ERR_CHK ( RS4_Add_LabelName ( &ec, sec->rfs_File, EndLabel, labname ) )
		}
...
static enum RS4FuncStat
RS4AdjustRefDef ( enum RS4ErrorCode * errcode UNUSED, RS4FileSection * sec )
{
	RS4Label * parent;
	RS4Label * last;
	RS4Label * rl;
...
static enum RS4FuncStat
RS4LabAdjust ( enum RS4ErrorCode * errcode, RS4Trace * rt UNUSED, RS4Label * rl, S64 adr, S64 size, enum RS4LabelType type )
{
	enum RS4ErrorCode ec;
	enum RS4FuncStat  fs;
	// RS4FileHeader *fh;
...
static enum RS4FuncStat
RS4Decode_Struct ( enum RS4ErrorCode * errcode, RS4Trace * rt UNUSED, RS4Label * rl, S64 * l )
{
	enum RS4ErrorCode ec;
	enum RS4FuncStat  fs;
...
bailout:
	...
	if ( l )
	{
		*l = ( ec == RS4ErrStat_Okay ) ? rl->rl_Size : 0;
	}
...
static enum RS4FuncStat
RS4Decode_String ( enum RS4ErrorCode * errcode, RS4Trace * rt UNUSED, RS4Label * rl, S64 max, S64 * l )
{
	enum RS4ErrorCode ec;
	enum RS4FuncStat  fs;
...
RS4Decode_RelativeWord ( enum RS4ErrorCode * errcode, RS4Trace * rt, RS4Label * rl, S64 pos, S64 * l )
{
	...
	for ( cnt = 0; cnt < size; cnt++ )
	{
		...
		ERR_CHK ( RS4_Find_LabelAdr ( &ec, rt->rt_File, &brance, adr, __FILE__ ) )
...
bailout:
	...
	if ( len_ptr )
	{
		*len_ptr = ( ec == RS4ErrStat_Okay ) ? len : 0;
	}
...
static enum RS4FuncStat
RS4Decode_Data ( enum RS4ErrorCode * errcode, RS4Trace * rt, RS4Label * rl, RS4Ref ** rr_ptr, S64 max, S64 * len_ptr )
{
	enum RS4ErrorCode ec;
	enum RS4FuncStat  fs;
	RS4Ref *		  rr;
...
bailout:
	...
	if ( len_ptr )
	{
		*len_ptr = ( ec == RS4ErrStat_Okay ) ? size : 0;
	}
...
static enum RS4FuncStat
Handle_Init ( enum RS4ErrorCode * errcode UNUSED, RS4Trace * rt UNUSED )
{
...
RS4Parser ( enum RS4ErrorCode * errcode, RS4Trace * rt )
{
	...
	while ( TRUE )
	{
		...
		switch ( type[pos] )
		{
			...
			case RS4MT_Code:
			{
				if ( ( DoVerbose > 2 ) && ( rt->rt_CurMemAdr & 1 ) )
				{
					printf ( "%s:%04d: Label Magic : Odd Address : $%" PRIx64 "\n", __FILE__, __LINE__, rt->rt_CurMemAdr );
				}
...
enum RS4FuncStat
RS4LabelMagic_File ( enum RS4ErrorCode * errcode, RS4FileHeader * fh )
{
	enum RS4ErrorCode ec;
	enum RS4FuncStat  fs;
	RS4FileSection *  sec;
