...
RS4AddSourceLine ( enum RS4ErrorCode * errcode, RS4Trace * rt )
{
...
#ifdef DEBUG
	if ( ( ! rt ) || ( ! rt->rt_Container.Hunk.ms_Str_Opcode ) || ( ! rt->rt_Container.Hunk.ms_Buf_Argument ) )
	{
...
static enum RS4FuncStat
RS4AddSourceString ( enum RS4ErrorCode * errcode, RS4Trace * rt, CSTR fmt, ... )
{
...
static enum RS4FuncStat
RS4AddEmptyLine ( enum RS4ErrorCode * errcode, RS4Trace * rt )
{
	enum RS4ErrorCode ec;
	enum RS4FuncStat  fs;
...
#ifdef DEBUG
	if ( ( ! rt ) || ( ! rt->rt_Container.Hunk.ms_Buf_Argument ) )
	{
		ec = RS4ErrStat_Internal;
		printf ( "%s:%04d: NULL Pointer\n", __FILE__, __LINE__ );
		printf ( "rt ................. : %p\n", rt );
...
static enum RS4FuncStat
__Struct_Ptr ( enum RS4ErrorCode * errcode, struct DataStructNode * dsn, RS4Trace * rt )
{
	enum RS4ErrorCode ec;
	enum RS4FuncStat  fs;
	RS4FileSection *  sec;
	...
	val = ( (U64)rt->rt_CurMemBuf[0] << 24 | (U64)rt->rt_CurMemBuf[1] << 16 | (U64)rt->rt_CurMemBuf[2] << 8
	...
	if ( ( sec->rfs_SecType == RS4ST_BSS ) || ( sec->rfs_MemoryAdr + sec->rfs_DataSize <= rt->rt_CurMemAdr ) )
	{
...
#ifdef DEBUG
			printf ( "%s:%04d: Error '%s' : %" PRIx64 " %" PRIx64 "\n", __FILE__, __LINE__, rl->rl_Name,
					 sec->rfs_DataSize + sec->rfs_MemoryAdr, rt->rt_CurMemAdr );
#endif
...
static enum RS4FuncStat
__Struct_Byte ( enum RS4ErrorCode * errcode, struct DataStructNode * dsn, RS4Trace * rt )
{
	enum RS4ErrorCode ec;
	enum RS4FuncStat  fs;
	RS4FileSection *  sec;
...
static enum RS4FuncStat
__Struct_Word ( enum RS4ErrorCode * errcode, struct DataStructNode * dsn, RS4Trace * rt )
{
	enum RS4ErrorCode ec;
	enum RS4FuncStat  fs;
	RS4FileSection *  sec;
	...
	val = ( (U64)rt->rt_CurMemBuf[0] << 8 | (U64)rt->rt_CurMemBuf[1] << 0 );
...
static enum RS4FuncStat
__Struct_Long ( enum RS4ErrorCode * errcode, struct DataStructNode * dsn, RS4Trace * rt )
{
	enum RS4ErrorCode ec;
	enum RS4FuncStat  fs;
	RS4FileSection *  sec;
...
static enum RS4FuncStat
RS4Decode_Struct ( enum RS4ErrorCode * errcode, RS4Trace * rt, RS4Label * rl, S64 off, S64 * lenptr )
{
...
static enum RS4FuncStat
RS4Decode_RelativeWord ( enum RS4ErrorCode * errcode, RS4Trace * rt, RS4Label * rl, S64 pos, S64 * l )
{
	enum RS4ErrorCode ec;
	enum RS4FuncStat  fs;
	RS4FileSection *  sec;
	...
	for ( cnt = 0; cnt < size; cnt++ )
	{
		...
		ERR_CHK ( RS4_Find_LabelAdr ( &ec, fh, &brance, adr, __FILE__ ) )
		...
		if ( parent )
		{
			...
			//			sprintf( rt->rt_Container.Hunk.ms_Buf_Argument, "(%s+%d", parent->rl_Name, brance->rl_Offset -
			// parent->rl_Offset );
		}
		else
		{
		...
		if ( parent )
		{
			...
			//			sprintf( & rt->rt_Container.Hunk.ms_Buf_Argument[p], "-%s+%d)", parent->rl_Name, rel->rl_Offset -
			// parent->rl_Offset );
		}
		else
		{
...
static enum RS4FuncStat
RS4Decode_Code ( enum RS4ErrorCode * errcode, RS4Trace * rt, RS4Label * rl, S64 size, S64 pos, MEM type, S64 * len_ptr )
{
...
static enum RS4FuncStat
RS4Decode_Data ( enum RS4ErrorCode * errcode, RS4Trace * rt, RS4Label * rl, RS4Ref * rr, S64 max, S64 * len_ptr )
{
	enum RS4ErrorCode ec;
	enum RS4FuncStat  fs;
	// RS4FileSection *sec;
	...
	if ( doptr )
	{
		...
		ERR_CHK ( RS4_Find_LabelAdr ( &ec, fh, &newrl, val, __FILE__ ) )
		...
		sprintf ( buf, "%02x%02x %02x%02x", rt->rt_CurMemBuf[0], rt->rt_CurMemBuf[1], rt->rt_CurMemBuf[2], rt->rt_CurMemBuf[3] );
	...
	else if ( max > 3 )
	{
		...
		{
			val = ( ( (U64)rt->rt_CurMemBuf[offset + 0] << 24 ) | ( (U64)rt->rt_CurMemBuf[offset + 1] << 16 )
					| ( (U64)rt->rt_CurMemBuf[offset + 2] << 8 ) | ( (U64)rt->rt_CurMemBuf[offset + 3] << 0 ) );
...
static enum RS4FuncStat
RS4Decode_String ( enum RS4ErrorCode * errcode, RS4Trace * rt, RS4Label * rl, S64 max, S64 * len_ptr )
{
	enum RS4ErrorCode ec;
	enum RS4FuncStat  fs;
...
static enum RS4FuncStat
RS4Decode_BSS ( enum RS4ErrorCode * errcode, RS4Trace * rt, S64 max, S64 * len_ptr )
{
	enum RS4ErrorCode ec;
	enum RS4FuncStat  fs;
	S64				  size;
...
static enum RS4FuncStat
Handle_Init ( enum RS4ErrorCode * errcode, RS4Trace * rt UNUSED )
{
	enum RS4ErrorCode ec;
	enum RS4FuncStat  fs;
	RS4FileSection *  sec;
...
static enum RS4FuncStat
Handle_Exit ( enum RS4ErrorCode * errcode,
...
static S64
Handle_Unset ( enum RS4ErrorCode * errcode,
			   ...
			   S64				   pos )  // mem/type pos
{
	...
	if ( ( sec->rfs_SecType == RS4ST_BSS ) || ( sec->rfs_DataSize <= pos ) )
	{
...
static S64
Handle_Code ( enum RS4ErrorCode * errcode,
			  RS4Trace *		  rt,	 // Trace info
			  RS4Label **		  rlptr, // Current Label
			  UNUSED RS4Ref **	  rrptr, // Current Ref
			  MEM				  type,	 // MemType
...
static S64
Handle_Data ( enum RS4ErrorCode * errcode,
			  RS4Trace *		  rt,	 // Trace info
			  RS4Label **		  rlptr, // Current Label
			  RS4Ref **			  rrptr, // Current Ref
										 //	MEM type,		// MemType
...
static enum RS4FuncStat
RS4Parser ( enum RS4ErrorCode * errcode, RS4Trace * rt )
{
	enum RS4ErrorCode ec;
	enum RS4FuncStat  fs;
	RS4FileSection *  sec;
...
enum RS4FuncStat
RS4BuildSource_File ( enum RS4ErrorCode * errcode, RS4FileHeader * fh )
{
	enum RS4ErrorCode ec;
	enum RS4FuncStat  fs;
	RS4Trace		  rt;
