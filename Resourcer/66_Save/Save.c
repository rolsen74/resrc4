...
static void
_GetStringInfo ( CSTR str, S32 * lenptr, S32 * posptr )
{
	S32 pos;
	S32 len;
	S32 chr;
...
S32
__Buf_WriteString ( struct Intern * in, STR buffer, U64 length )
{
	S32 error;
	U64 len;
...
static S32
__Line_Code ( struct Intern * in, RS4Source * rs )
{
	struct _SrcCode * sc;
	RS4Label *		  rl;
	char			  labname[64];
...
static S32
__Line_String ( struct Intern * in, RS4Source * rs )
{
...
static S32
__Line ( struct Intern * in, RS4Source * rs )
{
	S32 error;
...
__Header ( struct Intern * in )
{
	...
	sprintf ( in->SaveLineBuffer,
			  ...
			  "; MD5 ... : $%s\n"
			  ";\n",
			  VERSION, REVISION, DATE, fh->rfh_FileSize, ftype, fh->rfh_FileName, fh->rfh_FileMD5 );
...
static S32
__Split_Sec ( struct Intern * in, RS4FileSection * fs, S32 secnr )
{
	RS4Source * rs;
	STR			secname;
	S32			error;
...
enum RS4FuncStat
RS4SaveSource_File ( enum RS4ErrorCode * errcode, RS4FileHeader * fh, STR filename )
{
	struct Intern in;
