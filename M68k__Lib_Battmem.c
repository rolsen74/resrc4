...
static int Global_Battmem_Used = false;
...
{
	{ 0, -6, "_LVOObtainBattSemaphore", NULL },
	{ 0, -12, "_LVOReleaseBattSemaphore", NULL },
	{ 0, -18, "_LVOReadBattMem", NULL },
	{ 0, -24, "_LVOWriteBattMem", NULL },
	{ 0, 0, NULL, NULL }
};
...
int pos;
	...
	if ( lvo == NULL )
	{
printf( "Unknown Battmem libcall %d at $%08x\n", val, ms->ms_MemoryAdr );
	}
...
int len;
	...
	if ( error )
	{
		printf( "%s:%04d: Error Writting Battmem lvo's\n", __FILE__, __LINE__ );
		goto bailout;
	}
	...
	while( LVOS[pos].Name )
	{
		if ( LVOS[pos].Used )
		{
			...
			if ( error )
			{
				printf( "%s:%04d: Error Writting Battmem Data\n", __FILE__, __LINE__ );
				goto bailout;
			}
		}
