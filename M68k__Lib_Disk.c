...
static int Global_Disk_Used = false;
...
{
	{ 0, -6, "_LVOAllocUnit", NULL },
	{ 0, -12, "_LVOFreeUnit", NULL },
	{ 0, -18, "_LVOGetUnit", NULL },
	{ 0, -24, "_LVOGiveUnit", NULL },
	{ 0, -30, "_LVOGetUnitID", NULL },
	{ 0, -36, "_LVOReadUnitID", NULL },
	{ 0, 0, NULL, NULL }
};
...
int pos;
	...
	if ( lvo == NULL )
	{
printf( "Unknown Disk libcall %d at $%08x\n", val, ms->ms_MemoryAdr );
	}
...
int len;
	...
	if ( error )
	{
		printf( "%s:%04d: Error Writting Disk lvo's\n", __FILE__, __LINE__ );
		goto bailout;
	}
	...
	while( LVOS[pos].Name )
	{
		if ( LVOS[pos].Used )
		{
			...
			if ( error )
			{
				printf( "%s:%04d: Error Writting Disk Data\n", __FILE__, __LINE__ );
				goto bailout;
			}
		}
