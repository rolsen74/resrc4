...
static int Global_Translator_Used = false;
...
int pos;
	...
	if ( lvo == NULL )
	{
printf( "Unknown Translator libcall %d at $%08x\n", val, ms->ms_MemoryAdr );
	}
...
int len;
	...
	if ( Global_Translator_Used == false )
	{
		error = false;
		goto bailout;
	}
	...
	if ( error )
	{
		printf( "%s:%04d: Error Writting Translator lvo's\n", __FILE__, __LINE__ );
		goto bailout;
	}
	...
	while( LVOS[pos].Name )
	{
		if ( LVOS[pos].Used )
		{
			...
			if ( error )
			{
				printf( "%s:%04d: Error Writting Translator Data\n", __FILE__, __LINE__ );
				goto bailout;
			}
		}
