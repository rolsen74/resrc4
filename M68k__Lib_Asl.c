...
static int Global_Asl_Used = false;
...
static struct lvostruct LVOS[] =
{
	...
	{ 0, -48, "_LVOAllocAslRequest", NULL },
	{ 0, -54, "_LVOFreeAslRequest", NULL },
	{ 0, -60, "_LVOAslRequest", NULL },
	{ 0, 0, NULL, NULL }
};
...
int pos;
	...
	if ( lvo == NULL )
	{
printf( "Unknown Asl libcall %d at $%08x\n", val, ms->ms_MemoryAdr );
	}
...
int len;
	...
	if ( error )
	{
		printf( "%s:%04d: Error Writting Asl lvo's\n", __FILE__, __LINE__ );
		goto bailout;
	}
	...
	while( LVOS[pos].Name )
	{
		if ( LVOS[pos].Used )
		{
			...
			if ( error )
			{
				printf( "%s:%04d: Error Writting Asl Data\n", __FILE__, __LINE__ );
				goto bailout;
			}
		}
